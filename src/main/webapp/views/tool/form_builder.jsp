...
<script src="${pageContext.request.contextPath}/static/js/jquery-ui-1.10.4.min.js"></script>
<script src="${pageContext.request.contextPath}/static/js/plugins/beautifyhtml/beautifyhtml.js"></script>
<script>
    $(document).ready(function () {
        ...
        $("#copy-to-clipboard").on("click", function () {
            let $copy = $(".form-body").clone().appendTo(document.body);
            ...
            let html = html_beautify($copy.html());
            $copy.remove();
    ...
    let setup_draggable = function () {
        ...
        $(".droppable").droppable({
            ...
            drop: function (event, ui) {
                $(".empty-form").remove();
                let $orig = $(ui.draggable);
                if (!$(ui.draggable).hasClass("dropped")) {
                    let $el = $orig.clone().addClass("dropped").css({
                        "position": "static",
                        "left": null,
                        "right": null
                    }).appendTo(this);
                    let id = $orig.find(":input").attr("id");
                    if (id) {
                    ...
                    if ($(this)[0] != $orig.parent()[0]) {
                        let $el = $orig.clone().css({"position": "static", "left": null, "right": null}).appendTo(this);
                        $orig.remove()
                    }
                }
            }
    ...
    let get_modal = function (content) {
        let modal = $('<div class="modal" style="overflow: auto;" tabindex="-1">	<div class="modal-dialog"><div class="modal-content"><div class="modal-header"><a type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</a><h4 class="modal-title">编辑HTML</h4></div><div class="modal-body ui-front">	<textarea class="form-control" 	style="min-height: 200px; margin-bottom: 10px;font-family: Monaco, Fixed">' + content + '</textarea><button class="btn btn-success">更新HTML</button></div>				</div></div></div>').appendTo(document.body);
        return modal
    };
    $(document).on("click", ".edit-link", function (ev) {
        let $el = $(this).parent().parent();
        let $el_copy = $el.clone();
        let $edit_btn = $el_copy.find(".edit-link").parent().remove();
        let $modal = get_modal(html_beautify($el_copy.html())).modal("show");
        $modal.find(":input:first").focus();
        $modal.find(".btn-success").click(function (ev2) {
            let html = $modal.find("textarea").val();
            if (!html) {
                $el.remove()
            } else {
                $el.html(html);
